 ...
 * Desc   : 分页加载
 */
abstract class BaseVBListFragment<T, VB : ViewBinding, P : BasePresenter> :
    BaseVBFragment<VB, P>(), IRequestCommand, BaseListView<T> {
    ...
    protected open fun initLoadMoreConfig(): LoadMoreConfig {
        return LoadMoreConfig().apply {
            tag = this@BaseVBListFragment::class.java.simpleName
            // 加载数据的接口
            requestCommand = this@BaseVBListFragment
