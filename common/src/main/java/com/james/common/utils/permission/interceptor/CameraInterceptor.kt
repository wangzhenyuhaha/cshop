...
import com.james.common.utils.permission.callback.CameraCallback
 ...
 * Desc   : 定位权限
 */
class CameraInterceptor(val context: Context) : Interceptor() {

    override fun intercept(success: ((Any?) -> Unit)?, failed: ((Any?) -> Unit)?) {
        PermissionUtil.applyPermission(context, object : CameraCallback(context) {
