 ...
 * Desc   : 商品分组管理列表
 */
class CategoryAdapter(mList : List<CategoryVO>?) : BaseMultiItemQuickAdapter<CategoryVO, BaseViewHolder>(mList) {
    ...
    override fun convert(helper: BaseViewHolder, item: CategoryVO?) {
        item?.apply {
            if(item.itemType == TYPE_LEVEL_0) {
                ...
                helper.setImageResource(R.id.cateExpandIv, if(item.isExpanded) checkedDrawableRes else uncheckDrawable);
