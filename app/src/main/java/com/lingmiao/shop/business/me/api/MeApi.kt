...
//    APP修改店员信息
    ...
    suspend fun updatePersonInfo(@Path("id") id:String,@QueryMap map:Map<String,String?>): Response<ResponseBody>
...
//    发送手机号码
    ...
    suspend fun getSmsCode(@Path("mobile") mobile:String,@Body bean: CaptchaAli): Response<ResponseBody>

//    验证手机号码
    @GET("/seller/shops/clerks/smscode/{mobile}")
    @Headers("Content-type:application/json;charset=UTF-8")
    suspend fun checkSmsCode(@Path("mobile") mobile:String,@Query("sms_code") smsCode:String): Response<ResponseBody>
