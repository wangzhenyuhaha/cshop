...
abstract class DoubleItemPop<T : ItemData>(context: Context) :
    ...
    override fun onCreateContentView(): View = createPopupById(R.layout.common_pop_double_list)

    override fun onViewCreated(contentView: View) {
        popupGravity = Gravity.BOTTOM or Gravity.CENTER_VERTICAL

        // title
        popTitleTv = contentView.findViewById(R.id.doubleListTitleTv)

        // list
        level1Rv = contentView.findViewById(R.id.firstListRv)
        level2Rv = contentView.findViewById(R.id.secondListRv)
        ...
        // confirm
        popConfirmTv = contentView.findViewById(R.id.doubleListConfirmTv)

        // close
        popCloseTv = contentView.findViewById(R.id.doubleListCloseTv)
    ...
    private fun initRecyclerView(
        rv: RecyclerView?,
        lvAdapter: BaseQuickAdapter<T, BaseViewHolder>
    ) {
        rv?.apply {
            adapter = lvAdapter
    ...
    private fun initL2AdapterClick() {
        ...
        lv2Adapter.setOnItemClickListener { _, _, position ->
            run {
