 ...
 */
class GoodsStatusFragment : BaseLoadMoreFragment<GoodsVO, GoodsStatusPre>(),
    GoodsStatusPre.StatusView {

    companion object {
        const val KEY_GOODS_STATUS = "KEY_GOODS_STATUS"

        fun newInstance(goodsStatus: Int): GoodsStatusFragment {
            return GoodsStatusFragment().apply {
...
//                mPresenter?.clickItemView(mAdapter.getItem(position), position)
    ...
    override fun executePageRequest(page: IPage) {
        mPresenter?.loadListData(page, "", "", 0, mAdapter.data)
    }
